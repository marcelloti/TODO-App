...
begin
    ...
    for currentItem in TodoListBox.items do begin
      if (currentItem = itemNameToDelete) then begin
         ...
         //Application.MessageBox(PChar(itemToDelete), 'Match!');
      end;
      index := index + 1;
    end;
end;
...
begin
  ...
  if (itemsToDeleteCounter > 0) then begin
    ...
    if (Reply = true) then begin
      ...
      {
      for index in itemsToDeleteArrayOfNames do begin
        // Fix for some bug in for loop - PART 2
        listCounter := listCounter + 1;
        if ((listCounter > 1) AND (index = 0)) then begin
           Exit;
        end;

        TodoListBox.Items.Delete(index);
        Application.MessageBox(PChar(IntToStr(index)), 'Error');
      end;
      }
    end;
  end
